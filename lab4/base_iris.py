...
from lab4_header import scores_table as scores_table
from lab4_header import retrain_table as retrain_table
import pandas as pd
import numpy as np
...
def score( model_ID, feature_list, actual_class="-1" ):
    global models
    model = models[model_ID]
    ...
    response = scores_table.put_item(Item={
        'partition_key': now,
        'sort_key': f"score_{model_ID}",
        'Features': feature_str,
        'Class': str(iris_class),
...
def test(model_id, dataset_id):
    ...
    for i in range(len(X_test)):
        feature_str = ",".join([str(f) for f in X_test[i]])
        actual_class = str(actual[i])
        predicted_class = str(predicted[i])
        prob = str(np.max(y_pred[i])) 

        now = datetime.utcnow().strftime('%H:%M:%S.%f')[:-3]

        scores_table.put_item(Item={
            'partition_key': now,
            'sort_key': f"{model_id}_{dataset_id}_{i}",
            'Features': feature_str,
